...
$Type_stage =Type_stage::find(Input::get('type_id'));

$dateDebut = strtotime(Input::get('stage_dete_debut'));
$dateFin = strtotime(Input::get('stage_dete_fin'));
...
$min= date("Y-m-d",$dateDebut+($Type_stage->type_dur_min*86400));
 ...
 if( intval($dureeSejour)> intval($Type_stage->type_dur_max) || intval($dureeSejour)<  intval($Type_stage->type_dur_min) )
   ...
   if ($v->fails())
    ...
    $a1 = explode("-",Input::get('stage_dete_fin'));
...
$ent=null;
if(Input::get('entreprise_id')=='')
  $ent=null;
else
  $ent=Input::get('entreprise_id');
         ...
         $Stage = Stage::create(    
                                 array( 
                                         ...
                                         'entreprise_id'=>$ent,
                                         'stage_title'=>Input::get('stage_title'),
                                         'stage_dete_debut'=>Input::get('stage_dete_debut'),
                                         'stage_dete_fin'=>Input::get('stage_dete_fin'),
                                         'stage_nbr_etudiant'=>Input::get('stage_nbr_etudiant'),
                                         'stage_sujet'=>Input::get('stage_sujet'),
                                         'stage_annee_universitaire'=>$stage_annee,
                                         'stage_encadreur_s'=>Input::get('stage_encadreur_s'),
         ...
         $Stage->save();
                   ...
                   foreach(Input::get('filiere_id') as $filiere){
     ...
     /*    if(Input::get('entreprise_id')!='')
         { $Entreprise=Entreprise::find(Input::get('entreprise_id'));
                $data=['titre'=>'Bienvenu',
                      'content'=>'votre stage titulé '.Input::get('stage_title').' a été ajoutée avec succès',
...
*/
  ...
  {  
    ...
    $Soutenance=DB::select('select so.id "idSout", salle_id,  soute_date_debut, salle_nom, soute_date_fin 
...
if(Input::get('entreprise_id')=='')
  ...
  $ent=Input::get('entreprise_id');
         ...
         $Stage->type_id=Input::get('type_id');
         $Stage->entreprise_id = $ent;
         $Stage->stage_title = Input::get('stage_title');
         $Stage->stage_dete_debut = Input::get('stage_dete_debut');
         $Stage->stage_dete_fin = Input::get('stage_dete_fin');
         $Stage->stage_nbr_etudiant = Input::get('stage_nbr_etudiant');
         $Stage->stage_sujet = Input::get('stage_sujet');
         $Stage->stage_annee_universitaire = $stage_annee;
         $Stage->stage_encadreur_s = Input::get('stage_encadreur_s');
         $Stage->stage_status = Input::get('stage_status');
