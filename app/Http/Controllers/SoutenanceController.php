...
class SoutenanceController extends Controller {
     ...
     public function imprimer(Request $request)
  {
    ...
    $Ifiliere =Input::get('filiere');
    $Iniveau = Input::get('niveau');
    $Itypes =Input::get('types');
    $Ianner =Input::get('anner');
   

    $Stage=DB::select('select so.id "idSout", salle_id, stage_id, soute_date_debut,st.id, type_id , entreprise_id,stage_title,stage_encadreur_s,salle_nom,stage_annee_universitaire 
  ...
  { $Stage=DB::select('select so.id "idSout", salle_id, stage_id, soute_date_debut,st.id, type_id , entreprise_id,stage_title,stage_encadreur_s,salle_nom,stage_annee_universitaire from stage st , soutenance so,soutenance_salle sa where st.id = so.stage_id and so.salle_id = sa.id and so.soute_date_fin >"'.date('y-m-d h:i:s').'"');
    $jury=DB::select('select en.id,soutenance_id, qualite_id ,enseig_prenom, enseig_nom 
                      from enseignant en , soutenance_jury jr 
                      where en.id = jr.enseig_id');
    $Encadreur = DB::select('select * from encadreur e 
...
$Pose=$Configuration->soutenance_dur_max;
...
if( $test ==1)
 ...
 $Soutenance_salle = DB::select('select * from  soutenance_salle where id not in (select salle_id from salle_reserver where (date_debut between "'.$datet.'" and "'.$datefin.'" ) or (date_fin between "'.$datet.'" and "'.$datefin.'") )');
   
   $Enseignant = DB::select('select * from enseignant where (id in (select enseig_id from encadreur e where e.stage_id ="'.$id.'"))');
    $Encadreur = DB::select('select * from encadreur e where e.stage_id ="'.$id.'"');
   
   $jury = DB::select('select * from enseignant where (id not in (select enseig_id from encadreur e where e.stage_id ="'.$id.'")) and id not in (select enseig_id from soutenance_jury where soutenance_id in(select id from soutenance where (soute_date_debut between "'.$datet.'" and "'.$datefinEn.'" ) or (soute_date_fin between "'.$datet.'" and "'.$datefinEn.'")))');
 ...
 public function enregSoutenance($id)
  {

   $datet= Input::get('datedebut');
   $datefin=Input::get('datefin');
   $datefinEn=Input::get('datefinEn');
     $Soutenance_salle = DB::select('select * from  soutenance_salle where id not in (select salle_id from salle_reserver where (date_debut between "'.$datet.'" and "'.$datefin.'" ) or (date_fin between "'.$datet.'" and "'.$datefin.'") )');
      $jury = DB::select('select * from enseignant where (id not in (select enseig_id from encadreur e where e.stage_id ="'.$id.'")) and id not in (select enseig_id from soutenance_jury where soutenance_id in(select id from soutenance where (soute_date_debut between "'.$datet.'" and "'.$datefinEn.'" ) or (soute_date_fin between "'.$datet.'" and "'.$datefinEn.'")))');
    $idSalle=0;
foreach( $Soutenance_salle  as $salle)
 {
       if(Input::get('CBS'.$salle->id))
       {
             $idSalle=$salle->id;
