...
$image = Input::file('image');
 // $image = $request->file('Fichier');
  ...
  { 
    ...
    if ($v->fails())
    {
     ...
     $Etudiants = Etudiant::create(    
                                 array('etudiant_cin'=> Input::get('etudiant_cin'),
                                       'etudiant_mat'=> Input::get('etudiant_mat'), 
                                       'etudiant_prenom'=> Input::get('etudiant_prenom'),
                                       'etudiant_nom'=> Input::get('etudiant_nom'), 
                                       'etudiant_sexe'=> Input::get('etudiant_sexe'),
                                       'etudiant_date_nissance'=> Input::get('etudiant_date_nissance'), 
                                       'etudiant_lieu_naissance'=> Input::get('etudiant_lieu_naissance'),
                                       'etudiant_rue'=> '',
                                       'etudiant_ville'=> '',
                                       'etudiant_region'=> '', 
                                       'etudiant_image'=>'user.png',
                                       'etudiant_adresse'=> Input::get('etudiant_adresse'), 
                                       'etudiant_tel'=> Input::get('etudiant_tel'),
                                       'email'=> Input::get('email'), 
                                       'etudiant_rebouble'=> Input::get('etudiant_rebouble') ? 'Oui':'Nom', 
                                       'etudiant_moda'=> '', 
                                       'etudiant_filiere_id'=> Input::get('etudiant_filiere_id'),
                                       'etudiant_niveau'=> Input::get('etudiant_niveau'),
                                       'etudiant_group_code'=> Input::get('etudiant_group_code'),
                                       'etudiant_group_name'=> Input::get('etudiant_group_name')
                                      )
                                    );
  ...
  if ($v->fails())
   ...
   $etudiant->etudiant_cin= Input::get('etudiant_cin');
   $etudiant->etudiant_mat= Input::get('etudiant_mat') ;
   $etudiant->etudiant_prenom= Input::get('etudiant_prenom');
   $etudiant->etudiant_nom= Input::get('etudiant_nom');
   $etudiant->etudiant_sexe= Input::get('etudiant_sexe');
   $etudiant->etudiant_date_nissance= Input::get('etudiant_date_nissance');
   $etudiant->etudiant_lieu_naissance= Input::get('etudiant_lieu_naissance');
   $etudiant->etudiant_rue= '';
   $etudiant->etudiant_ville= '';
   $etudiant->etudiant_region= '';
   $etudiant->etudiant_adresse= Input::get('etudiant_adresse'); 
   $etudiant->etudiant_tel= Input::get('etudiant_tel');
   $etudiant->email= Input::get('email'); 
   $etudiant->etudiant_rebouble= Input::get('etudiant_rebouble');  
   $etudiant->etudiant_moda= ''; 
   $etudiant->etudiant_filiere_id= Input::get('etudiant_filiere_id');
   $etudiant->etudiant_niveau= Input::get('etudiant_niveau');
   $etudiant->etudiant_group_code= Input::get('etudiant_group_code');
   $etudiant->etudiant_group_name= Input::get('etudiant_group_name');
   $etudiant->save();
